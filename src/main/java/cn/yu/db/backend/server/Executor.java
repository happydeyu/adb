package cn.yu.db.backend.server;

import cn.yu.db.backend.parser.Parser;
import cn.yu.db.backend.parser.statement.*;
import cn.yu.db.backend.tbm.BeginRes;
import cn.yu.db.backend.tbm.TableManager;
...
public class Executor {
    ...
    public byte[] execute(byte[] sql) throws Exception {
        ...
        if(Begin.class.isInstance(stat)) {
            if(xid != 0) {
                throw DbException.NestedTransactionException;
            }
            ...
            if(xid == 0) {
                throw DbException.NoTransactionException;
            }
