...
public class Table {
    ...
    private Table parseSelf(byte[] raw) {
        ...
        name = res.str;
        position += res.next;
        nextUid = ByteUtil.byteToLong(Arrays.copyOfRange(raw, position, position+8));
        position += 8;

        while(position < raw.length) {
            long uid = ByteUtil.byteToLong(Arrays.copyOfRange(raw, position, position+8));
            position += 8;
    ...
    public int update(long xid, Update update) throws Exception {
        ...
        if(fd == null) {
            throw DbException.FieldNotFoundException;
        }
    ...
    private Map<String, Object> string2Entry(String[] values) throws Exception {
        if(values.length != fields.size()) {
            throw DbException.InvalidValuesException;
        }
        ...
        if(where == null) {
            ...
            for (Field field : fields) {
                if(field.fieldName.equals(where.singleExp1.field)) {
                    if(!field.isIndexed()) {
                        throw DbException.FieldNotIndexedException;
                    }
                    fd = field;
                    break;
                }
    ...
    private CalWhereRes calWhere(Field fd, Where where) throws Exception {
        ...
        switch(where.logicOp) {
            ...
            case "and":
                ...
                break;
            default:
                throw DbException.InvalidLogOpException;
        }
        return res;
    }
